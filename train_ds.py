...
def parse_args(args):
    ...
    # sem_seg||refer_seg||vqa||reason_seg
    parser.add_argument(
        "--dataset", default="sem_seg||refer_seg||vqa||reason_seg", type=str
    )
    parser.add_argument("--sample_rates", default="10,6,1,2", type=str) # 9,6,1,2
    # cocostuff||ade20k||mapillary||cityscapes||openimage||pascal_part||paco_lvis
    parser.add_argument(
        "--sem_seg_data",
        default="cocostuff||ade20k||mapillary||cityscapes||openimage", type=str)
    ...
    parser.add_argument("--val_dataset_list", default="refcoco|unc|val,refcoco|unc|testA,refcoco|unc|testB,refcoco+|unc|val,refcoco+|unc|testA,refcoco+|unc|testB,refcocog|umd|val,refcocog|umd|test", type=list_of_str) #  refcoco|unc|val  refcoco+|unc|val  refcocog|umd|val
    ...
    parser.add_argument("--num_all_classes", default=80, type=int)
    parser.add_argument("--exclude_val", action="store_true", default=False)
    parser.add_argument("--no_eval", action="store_true", default=False)
    parser.add_argument("--eval_only", action="store_true", default=False)
    parser.add_argument("--final_eval", action="store_true", default=True)
    parser.add_argument("--vision_pretrained", default="./checkpoint/SAM/sam_vit_h_4b8939.pth", type=str)
...
def main(args):
    ...
    if args.auto_resume and len(args.resume) == 0:
        for kk in range(args.epochs):
            resume = os.path.join(args.log_dir, f"epoch{kk}_model")
    ...
    if args.eval_only:
        for k in range(len(args.val_dataset_list)):
            print(f'-----------{args.val_dataset_list[k]} result----------')
            giou, ciou = validate(val_loader_list[k], model_engine, 0, writer, args)
        exit()
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch == args.epochs-1:
            save_dir = os.path.join(args.log_dir, "final_model")
            if args.local_rank == 0:
        ...
        # final_eval 
        if args.final_eval and epoch == args.epochs-1:
            for k in range(len(args.val_dataset_list)):
                cur_val_dataset = ValDataset(
                    args.dataset_dir,
                    tokenizer,
                    args.vision_tower,
                    args.val_dataset_list[k],
                    args.image_size,)
                cur_val_sampler = torch.utils.data.distributed.DistributedSampler(cur_val_dataset, shuffle=False, drop_last=False)
                cur_val_loader = torch.utils.data.DataLoader(
                    cur_val_dataset,
                    batch_size=args.val_batch_size,
                    shuffle=False,
                    num_workers=args.workers,
                    pin_memory=False,
                    sampler=cur_val_sampler,
                    collate_fn=partial(
                        collate_fn,
                        tokenizer=tokenizer,
                        conv_type=args.conv_type,
                ...
                if args.local_rank == 0:
                    print(f'-----------{args.val_dataset_list[k]} result----------')
                giou, ciou = validate(cur_val_loader, model_engine, 0, writer, args)
        


        if is_best:
            if epoch>0:
                save_dir = os.path.join(args.log_dir, f"epoch{epoch}_model")
