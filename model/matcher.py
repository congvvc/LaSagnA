...
class HungarianMatcher(nn.Module):
    ...
    def __init__(self, cost_class: float = 1, cost_mask: float = 1, cost_dice: float = 1, num_points: int = 0,
                 cost_box: float = 5.0, cost_giou: float = 2.0, panoptic_on: bool = False):
        """Creates the matcher
...
if __name__ == "__main__":
    ...
    pred_box = torch.tensor([[3,66,88,99], [1, 2, 21, 26], [3,66,88,199], [3,66,88,109]], dtype=torch.float32)
    gt_box = torch.tensor([[1, 2, 35, 36], [3,68,88,99], [3,68,88,99]], dtype=torch.float32)

    matcher = HungarianMatcher(cost_box=5.0, cost_giou=2.0)

    pred_match_indx, gt_match_indx= matcher(pred_box, gt_box)

    pre_box_match = pred_box[pred_match_indx]

    gt_box_match = gt_box[gt_match_indx]

    new_match = matcher(pre_box_match, gt_box_match)
